...
class AdState with ChangeNotifier {
  ...
  AdState() {
    _initializationFuture = _initializeAdState(); // 초기화 메서드 호출 및 Future 캐싱
  }
  ...
  set adsRemoved(bool value) {
    _adsRemoved = value;
    adsRemovedGlobal = value; // Sync with global variable
  ...
  Future<void> _initializeAdState() async {
    ...
    adsRemovedGlobal = _adsRemoved;

    print('[AdState] initialized with adsRemoved: $_adsRemoved, adsRemovedGlobal: $adsRemovedGlobal');
  ...
  // 초기화 완료 여부를 확인하기 위한 Future (캐싱된 Future 반환)
  Future<bool> get isInitialized => _initializationFuture.then((_) => true);
}