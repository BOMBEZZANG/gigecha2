...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  // Purchase status is now initialized by AdState directly
  // This method only syncs the global variable from AdState after initialization
  Future<void> _initializePurchaseStatus() async {
    final adState = Provider.of<AdState>(context, listen: false);
    await adState.isInitialized;
    // adsRemovedGlobal is now synced automatically by AdState
  }
  ...
  Future<void> _requestTrackingPermission(BuildContext context) async {
    ...
    bool hasRequestedTracking =
        prefs.getBool('has_requested_tracking') ?? false;
        ...
        actions: [
          ...
          TextButton(
            ...
            onPressed: () {
              ...
              _showRewardedAdAndNavigateToDictionary();
            },
          ),
        ],
      ),
  ...
  void _navigateToDictionary() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => DictionarySelectPage()),
    );
  }
  ...
  void _navigateToOXQuiz() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => OXQuizPage(category: "전체문제")),
    );
  }
            ...
            child: SlideTransition(
              ...
              child: Column(
                children: [
                  ...
                  if (showBanner) _buildModernBanner('광고 제거', isDarkMode),
                ],
              ),
            ),
          ),
            ...
            Expanded(
              child: _buildFeatureCard(
                ...
                onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => QuestionSelectPage())),
              ),
            ),
            SizedBox(width: 12),
            Expanded(
            ...
            Expanded(
              child: _buildFeatureCard(
                ...
                onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => RandomQuestionSelectPage())),
              ),
            ),
            SizedBox(width: 12),
            Expanded(
    ...
    return Column(
      children: [
        _buildWideFeatureCard(
          title: '용어사전',
          subtitle: '핵심 용어를 학습 및 암기',
          icon: Icons.menu_book_rounded,
          color: Color(0xFF6366F1),
          onTap: _showDictionaryWithAd,
        ),
        SizedBox(height: 12),
        _buildWideFeatureCard(
          title: '기출 음성듣기',
          ...
          onTap: () => Navigator.push(context,
              MaterialPageRoute(builder: (context) => AudioSelectPage())),
        ),
        SizedBox(height: 12),
        _buildWideFeatureCard(
          title: '핵심강의 듣기',
          ...
          onTap: () => Navigator.push(context,
              MaterialPageRoute(builder: (context) => SummarySelectPage())),
        ),
        SizedBox(height: 12),
        _buildWideFeatureCard(
          title: '학습 통계보기',
  ...
  Widget _buildWideFeatureCard({
    ...
    // Overflow 에러 해결을 위해 height: 80을 제거하거나 null로 설정하여
    // 컨텐츠 크기에 따라 유연하게 늘어나도록 수정합니다.
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(context,
            MaterialPageRoute(builder: (context) => AdRemovePage())).then((_) {
          _initializePurchaseStatus();
        });
      },
      child: Container(
          ...
          children: [
            ...
            _buildDrawerItem(
              ...
              onTap: () {
                Navigator.pop(context);
                Navigator.push(context,
                        MaterialPageRoute(builder: (context) => AdRemovePage()))
                    .then((_) {
                  _initializePurchaseStatus();
                });
              },
