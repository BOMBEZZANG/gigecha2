...
class _AdRemovePageState extends State<AdRemovePage> with TickerProviderStateMixin {
  ...
  bool _loading = true;
  final String _adRemovalProductId = 'ad_remove_jigejongmin'; // 광고 제거 상품 ID
  ...
  Future<void> _purchaseAdRemoval() async {
    if (_products.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("상품 정보를 불러올 수 없습니다. 네트워크 연결을 확인해주세요."),
    ...
    if (product != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("상품을 찾을 수 없습니다. 잠시 후 다시 시도해주세요."),
          backgroundColor: Colors.red,
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
        ),
  ...
  Future<void> _restorePurchases() async {
    ...
    // 복원된 구매가 없고, 아직 광고 제거가 안 된 상태라면 알림
    if (!_restoredAny && !_adRemoved && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  void _handleAdRemovalSuccess(PurchaseDetails purchase) async {
    ...
    // Update global AdState provider
    if (mounted) {
      final adState = Provider.of<AdState>(context, listen: false);
      adState.adsRemoved = true;
    }

    // Update global variable for immediate effect
    adsRemovedGlobal = true;
